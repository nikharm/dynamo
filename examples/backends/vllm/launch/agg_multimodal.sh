...
# Multimodal VL models have high CUDA graph + torch.compile overhead, so we use
# a fixed 0.85 fraction (empirically validated) rather than the dynamic estimator.
# DYN_GPU_MEMORY_FRACTION_OVERRIDE overrides the fraction (used by profiler).
GPU_MEM=${DYN_GPU_MEMORY_FRACTION_OVERRIDE:-0.85}
